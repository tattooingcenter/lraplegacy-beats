...
## 試聴の仕組み
- 非会員は各ビートの **7秒だけ試聴**できます。フルDL・フル再生は会員のみ。試聴は投稿時に自動生成。

## ビートを増やすとき（v2）
- `/upload.html` からチームの各自が合言葉でログインしてアップロード → `/admin.html` であなたが承認 → 公開。
- GitHubやコード編集は不要。詳しくは SETUP_v2.md を参照。
- （v1の `beats/` フォルダ方式はv2では使いません）
